...
import { FormData, TypePropsModal, updateUser } from "./api";
...
export default function UpdateUserModal({ close, onClose ,user}: TypePropsModal) {
  ...
  const onSubmit = (dataS: any) => {
    mutation.mutate({...dataS, id: user.id});
  };
  ...
  return (
    <Dialog open={close} onOpenChange={onClose}>
      <DialogContent >
        <DialogHeader>
          <DialogTitle className="text-textColor">Chỉnh sửa người dùng này</DialogTitle>
        </DialogHeader>
        {
            ...
            <form onSubmit={handleSubmit(onSubmit)} method="POST" className="grid gap-4 py-4">
              ...
              <div className="flex flex-col gap-[5px] ">
                <div className="flex items-center gap-4">
                  ...
                  <Select value={gender} onValueChange={(value: "Male" | "Female" | "Other") => setValue("gender", value)}>
              ...
              <Button className="bg-primaryColor hover:bg-primaryColor transition-all duration-300 ease-linear hover:opacity-80" type="submit">Sửa người dùng</Button>
            </form>)
        }
      </DialogContent>
    </Dialog >
