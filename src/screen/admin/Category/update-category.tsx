...
import { FormData, TypePropsModal, updateCat } from "./api";
...
export default function UpdateCategoryModal({ category, close, onClose }: TypePropsModal) {
  ...
  const onSubmit = (dataS: any) => {
    mutation.mutate({...dataS,catId:category.id})
  }
  useEffect(() => {
    console.log(data)
    if (isSuccess) {
  ...
  return (
    <Dialog open={close} onOpenChange={onClose} >
      <DialogContent >
        <DialogHeader>
          <DialogTitle className="text-textColor">Sửa danh mục danh mục</DialogTitle>
        </DialogHeader>
        {
          isPending ? (<LoadingSpinner className='my-[70px] mx-auto ' />) : (<form onSubmit={handleSubmit(onSubmit)} className="grid gap-4 py-4">
